...
{
	document.getElementById(verseDivID).style.backgroundColor = "#ffff00";
}
...
function onComment(bookID, chapterID, verseNumber, verseDivPretext) {
	
	var commentID = "CommentDivID_" + verseNumber;
	var commentTextID = "CommentTextDivID_" + verseNumber;
	var commentTextName = "CommentText_" + verseNumber;
	var commentDiv = document.getElementById(commentID);
	var verseDiv = document.getElementById(verseDivPretext + verseNumber);
	...
	if (!commentDiv) //create comment div
	{
		if (verseDiv)
		{
			...
			var cancelBtnDivStr = "<span style=\"padding-left:.2em; margin-bottom:20px; margin-left:5px;\"><button onclick=\"javascript:onCancelCommentClick('" + verseDivPretext + "');\" class=\"btn\"><span><span>&nbsp&nbsp Cancel &nbsp&nbsp</span></span></button></span>";
			var postBtnDivStr = "<span style=\"margin-bottom:20px; margin-left:20px;\"><button onclick=\"javascript:onPostCommentClick('" + verseDivPretext + "');\" class=\"btn\"><span><span>&nbsp&nbsp&nbsp&nbsp Post &nbsp&nbsp&nbsp&nbsp</span></span></button></span>";
			...
			adjustCommentTextBox(verseNumber);
			if (gPrevVerseNumber)
			{
				var prevVerseDiv = document.getElementById(verseDivPretext + gPrevVerseNumber);
				var preCommentDiv = document.getElementById("CommentDivID_" + gPrevVerseNumber);
...
function presentBible(tabContainer, booksDivID, chaptersDivID, versesDivID, versesDivIDASV, 
	footerDivID, loginDivID, loginBoxDivID, facebookBtnDivID, 
	diggBtnDivID, googleBtnDivID, tweetBtnDivID, twitterBtnDivID, maximizeBtnDiv, highlightVerseBtnID)
{
	$gtabContainer = tabContainer;
	$gbooksDivID = booksDivID;
	$gchaptersDivID = chaptersDivID;
	$gversesDivID = versesDivID + "_Content";
	$gversesDivIDASV = versesDivIDASV + "_Content";
	$gcontentHeaderDivID = versesDivID + "_Title";
	$gcontentHeaderDivIDASV = versesDivIDASV + "_Title";;
...
{
	if ($gloginButtonDivID && $gloginButtonDivID != "")
	{
...
{	
	...
	if (twitterBtnDiv)
	{
		...
		var twitterHtmlText = "<a href=\"http://www.twitter.com/cloudstudybible\" target=\"_blank\"><img src=\"http://twitter-badges.s3.amazonaws.com/t_small-a.png\" alt=\"Follow Cloud Study Bible on Twitter\" style=\"padding-top:2px; border:none;\"/></a>";
...
function searchBible(searchText, searchTabBarID, searchContentDivID)
{
	...
	if ($gtabContainer != null)
		selectTab($gtabContainer, "Search");
...
function loadCurrentChapter() {
    ...
    loadVerses($gcurrentBookName, $gcurrentBookID, $gcurrentChapterNumber, 1, $gcurrentVerseNumber, 0);
    scrollToBook($gcurrentBookID);
    scrollToChapter($gcurrentChapterNumber);
} //loadCurrentChapter()

function goToVerse(bookName, bookID, chapterNumber, verseNumber, fromSearch) {
    loadBookChapterNumbers(bookID, bookName, 1, chapterNumber, verseNumber, fromSearch);
...
function scrollToVerseAsv(verseNumber) {
	...
	if (verseDiv)
	{
		...
		if ($verseDivHeight > 0)
			document.getElementById($gversesDivIDASV).scrollTop = $verseDivTop - ($verseDivHeight/2); //search added to the content div
	}
} //scrollToVerseAsv()


// Prevent tab from scrolling to top on selection.
function onTabSelection(event, ui)
{
	if ($gcurrentVerseNumber > 1)
	{
		if (ui.tab.id == "kjvTabId")
			...
			scrollToVerseAsv($gcurrentVerseNumber);
	}
	return true;
}
...
function loadVersesAsv(bookName, bookID, chapterNumber, scrollToDiv, scrollToVerseID, fromSearch)
{
...
function loadVerses(bookName, bookID, chapterNumber, scrollToDiv, scrollToVerseID, fromSearch)
{
	$gcurrentBookName = bookName;
	$gcurrentBookID = bookID;
	$gcurrentChapterNumber = chapterNumber;
	...
	$gxmlhttpVerses.onreadystatechange=(function stateChanged() { 
		if ($gxmlhttpVerses.readyState==4) {
			...
			if ($gcurrentVerseNumber < 1)
				$gcurrentVerseNumber = 1;
			loadVersesAsv(bookName, bookID, chapterNumber, scrollToDiv, $gcurrentVerseNumber, fromSearch);
		}
		else {
			ClearVerses();
...
function loadBookChapterNumbers(bookID, bookName, scrollToDiv, gotoChapter, gotoVerseNumber, fromSearch) 
{
	...
	$gxmlhttpBookChapters.onreadystatechange = (function chapterCountReceived() {
		if ($gxmlhttpBookChapters.readyState==4) {
			...
			for (i = 1; i <= count; i++) 
			{
				...
				$onClickStr = "onclick=\"javascript:loadVerses('" + $gcurrentBookName + "', '" + $gcurrentBookID + "', '" + i + "','0','1','" + 0 + "');\" ";
				$chNo = i;
				...
				$hrefStr = "href=\"javascript: loadVerses('" + $gcurrentBookName + "', '" + $gcurrentBookID + "', '" + i + "','1','1','0','" + 0 + "');\"";
			...
			loadVerses($gcurrentBookName, $gcurrentBookID, $gcurrentChapterNumber, 1, $gcurrentVerseNumber,fromSearch);
