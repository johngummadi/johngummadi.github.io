...
function getContent()
{
	...
	openDBConnection();
	$sql0="SELECT FullName FROM WebBooks WHERE ID='$bookNumber'";
    //echo $sql0;
	$result0=mysql_query($sql0);
	$bookName=mysql_result($result0,0,"FullName");
    
    
	$sql1="SELECT ID FROM WebChapters WHERE BookID='$bookNumber' AND ChapterNumber='$chapterNumber'";
    //echo $sql1;
	...
	if ($verseNumber>0)
		$sql2="SELECT * FROM WebVerses WHERE ChapterID='$chapterID' AND VerseNumber='$verseNumber'";
	else
		$sql2="SELECT * FROM WebVerses WHERE ChapterID='$chapterID'";
    //echo $sql2;
	...
	while ($index < $rowCount) {
		...
		$mouseClickEventStr = "onClick=\"javascript:clickOnVerse('webVerseId$divID');\" ";
		$mouseOverEventStr = "onMouseOver=\"javascript:mouseOnVerse('webVerseId$divID');\" ";
		$mouseOutEventStr = "onMouseOut=\"javascript:mouseOffVerse('webVerseId$divID');\" ";
		...
		$newcommentImgStr = "";
		$mouseOverCommentsStr = "onMouseOver=\"javascript:onMouseOverCommentBtn('WebNewCommentBtnId$divID');\" ";
		$mouseOutCommentsStr = "onMouseOut=\"javascript:onMouseOutCommentBtn('WebNewCommentBtnId$divID');\" ";
		$facebookImageDivStr = "<span> <img id=\"WebFBBtnId$divID\" style=\"position:relative; top:2px;\" onclick=\"javascript:onFacebookBtnClicked('$bookName $chapterNumber:$verseNumber (WEB)', '$verse', '');\" onMouseOver=\"style.cursor='pointer';\" src=\"biblejs/images/fb.png\" height=\"17\" width=\"17\" title=\"Share $bookName $chapterNumber:$verseNumber with your friends on the facebook\"/> &nbsp;&nbsp;&nbsp;&nbsp; </span>";
		
		if ($rowCountComments > 0) // when comment(s) exist
		{
			$newcommentImgStr = "<span id=\"WebNewCommentBtnId$divID\" verseiddb=\"$verseID\" bookName=\"$bookName\" chapterNumber=\"$chapterNumber\" verseNumber=\"$verseNumber\" commentCount=\"$rowCountComments\" $mouseOverCommentsStr $mouseOutCommentsStr title=\"$rowCountComments comment(s) on $bookName $chapterNumber:$verseNumber, click to view or add a new comment on this verse.\" onclick=\"javascript:onShowComments('$bookNumber','$chapterID','$verseID','$verseNumber','$divID', 'webVerseId');\" style=\"background-image:url(biblejs/images/comments_show_small.png); background-repeat:no-repeat; background-position:center center;padding:.1em;\"> <span style=\"position:relative; top:-4px; font-size:9px; font-weight:bold; color:#aa0000;\">&nbsp;&nbsp; $rowCountComments &nbsp;&nbsp;</span></span>";
		}
		...
		$line = "<div id=\"webVerseId$divID\" verse=\"$verse\" $mouseClickEventStr $mouseOverEventStr $mouseOutEventStr $styleStr\"><span style=\"color:#000099;\">$divID.</span> \t $verse $newcommentImgStr $showCommentsImgStr $facebookImageDivStr </div>";
